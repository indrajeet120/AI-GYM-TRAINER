...
#      try:
#         tts = TextToSpeech()
#         audio = tts.speak("Testing AI gym coach audio")
...
#     # Play test audio
#     if  st.session_state.get("test_audio"):
#       st.audio(st.session_state.test_audio, format="audio/mp3")
#     #  audio = tts.speak("Testing AI gym coach audio")
#     #  st.audio(audio, format="audio/mp3")
...
#         )
#         if workout_started and not st.session_state.get("audio_to_play"):
#           st_autorefresh(interval=5000, key="gym_refresh")
#         # Update metrics only when processor is ready
...
#              # 🔊 Voice trigger when rep increases
#             current_reps = st.session_state.get("reps", 0)
#             last_spoken = st.session_state.get("last_spoken_rep", 0)

#             if current_reps > last_spoken:
#               if st.session_state.voice_pipeline:
#                 result = st.session_state.voice_pipeline.process_event(
#                   event="rep_completed",
#                   exercise=st.session_state.get("exercise_type"),
...
# # Auto refresh UI while workout is running
...
#         #   st.rerun()
#         # if workout_started:
#         #   st_autorefresh(interval=1500, key="gym_refresh") 
#         # Auto refresh only when no audio is playing
#         # if workout_started and not st.session_state.get("audio_to_play"):
#         #   st_autorefresh(interval=5000, key="gym_refresh")
